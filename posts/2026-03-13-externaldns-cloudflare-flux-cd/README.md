...
- A CloudFlare DNS account with DNS management access
- A domain hosted in CloudFlare DNS
...
ExternalDNS deployed via Flux CD provides automated, GitOps-managed DNS record lifecycle for Kubernetes services. By combining ExternalDNS's CloudFlare DNS provider with Flux CD's reconciliation, you get a system where DNS records are automatically created when services are deployed and removed when they are deleted, with all configuration tracked in Git.
