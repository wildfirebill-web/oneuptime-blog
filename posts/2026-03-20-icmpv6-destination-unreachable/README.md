...
The "Unused" 4 bytes are set to zero by the sender and ignored
by the receiver (reserved for future use).
```
...
Code 3: Address unreachable
  → Specific destination address is unreachable at the link layer
  → Neighbor Discovery failed (no MAC for the IPv6 address)
  → Equivalent to IPv4 "host unreachable"

Code 4: Port unreachable
  → Destination host received the packet but no process is listening
  → Generated by the destination, not by a router
  → Direct equivalent of IPv4 ICMP Port Unreachable
...
Code 7: Error in Source Routing Header
  → Source Routing Header caused a forwarding error
  → Used with Routing Header Type 4 (Segment Routing)
```
...
ICMPv6 Destination Unreachable covers a range of failure scenarios through its Code field. Code 0 indicates routing failure, Code 1 indicates policy block (firewall), Code 3 indicates link-layer resolution failure (ARP equivalent), and Code 4 indicates no service on the destination port. When troubleshooting IPv6 connectivity, the specific Code value immediately identifies the failure category: routing (Code 0), security policy (Code 1), NDP failure (Code 3), or application not running (Code 4).
