# Using the Calico HostEndpoint Resource in Production Clusters

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Calico, Kubernetes, Endpoint

Description: Practical patterns and real-world examples for using Calico HostEndpoint resources effectively in production Kubernetes environments.

---

## Introduction

Knowing how to create a Calico HostEndpoint resource is only the first step. Using it effectively in production requires understanding common patterns, combining it with other Calico resources, and adapting it to real-world scenarios.

This guide presents practical use cases for the HostEndpoint resource, drawn from common production Kubernetes deployments. Each example includes guidance and an explanation of why the configuration is structured that way.

Whether you are running a small cluster or a large multi-tenant environment, these patterns will help you get the most out of the HostEndpoint resource.

## Prerequisites

- A running Kubernetes cluster with Calico (v3.26+)
- `kubectl` and `calicoctl` installed
- Basic understanding of the HostEndpoint resource fields (see our creation guide)

## Pattern 1: Default Configuration for Small Clusters

For clusters with fewer than 50 nodes, a straightforward HostEndpoint configuration works well:
...
The HostEndpoint resource works together with other Calico resources. Here is how to verify the combined effect:
...
Set up ongoing monitoring for your HostEndpoint resources:

```bash
# Watch for changes to HostEndpoint resources
kubectl get hostendpoint.projectcalico.org -w
...
After configuring the HostEndpoint resource for your production use case, run a comprehensive check:
...
- Verify the resource is correctly applied: `calicoctl get hostendpoint -o yaml`.
