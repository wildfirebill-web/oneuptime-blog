# Creating the Calico GlobalNetworkPolicy Resource in Kubernetes

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Calico, Kubernetes, Network Policy

Description: Learn how to define and apply a Calico GlobalNetworkPolicy resource to your Kubernetes cluster using kubectl and calicoctl.

---

## Introduction

Calico uses custom Kubernetes resources to configure networking and security in your cluster. The GlobalNetworkPolicy resource is one of these building blocks, and understanding how to create it properly is essential for any Kubernetes operator running Calico.

This guide walks you through defining a GlobalNetworkPolicy manifest, understanding each field, and applying it to your cluster. Whether you are setting up a new cluster or extending an existing Calico deployment, you will learn the correct way to create this resource.

By the end of this post you will have a working GlobalNetworkPolicy resource applied to your cluster, with a clear understanding of what each field controls and how to verify that the resource is active.
...
The GlobalNetworkPolicy resource uses the Calico API group `projectcalico.org/v3`. Before writing the manifest, review the key fields:

- `order`: Determines the evaluation order. Lower numbers are evaluated first.
- `selector`: A Calico label selector that determines which endpoints this policy applies to.
- `types`: Which traffic directions this policy covers: `Ingress`, `Egress`, or both.
- `egress` / `ingress`: Lists of rules with an `action` (Allow, Deny, Log, Pass) and optional match criteria.

## Creating the GlobalNetworkPolicy Manifest

Create a file named `globalnetworkpolicy.yaml` with the following content:
...
You have created a Calico GlobalNetworkPolicy resource, applied it to your cluster, and verified it is active. This resource is a foundational piece of your Calico configuration. Keep your manifests in version control and validate changes with `calicoctl` before applying to production clusters.
