# Configuring Implementation Modes in Cilium Networking

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Cilium, Kubernetes, Networking

Description: Learn how to configure Cilium networking implementation modes including VXLAN overlay, Geneve overlay, and native routing in your Kubernetes cluster for optimal performance and security.

---

## Introduction

Proper configuration of implementation modes in cilium networking is essential for a production-ready Cilium deployment. Cilium supports multiple networking implementation modes: VXLAN overlay (default), Geneve overlay, and native/direct routing. Each mode has different requirements and trade-offs. VXLAN and Geneve create overlay networks that encapsulate pod traffic between nodes. Native routing uses the existing network infrastructure to route pod traffic directly.
...
Apply the following Helm values to configure implementation modes in cilium networking:
...
# Test workload to verify implementation modes in cilium networking configuration
apiVersion: apps/v1
kind: Deployment
metadata:
  name: config-test
