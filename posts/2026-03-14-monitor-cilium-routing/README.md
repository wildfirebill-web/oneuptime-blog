# Monitoring Cilium Routing

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Cilium, Kubernetes, Networking, Monitoring

Description: Set up monitoring and alerting for Cilium routing configuration and operations including tunnel and native routing modes to detect issues before they impact application availability.

---

## Introduction

Monitoring cilium routing provides early warning when configuration drift, resource exhaustion, or component failures affect networking. Cilium routing determines how pod-to-pod traffic is forwarded between nodes. In tunnel mode, Cilium encapsulates packets in VXLAN or Geneve headers. In native routing mode, Cilium programs routes directly into the Linux kernel routing table and relies on the underlying network to forward packets between nodes.
...
This guide covers metrics collection, dashboard creation, and alert configuration for cilium routing.
...
## Key Metrics for Cilium Routing
...
Create Prometheus alerts for cilium routing:

```yaml
# cilium-feature-alerts.yaml
# Alerting rules for cilium routing
      ...
      rules:
        - alert: CiliumAgentUnhealthy
          ...
          annotations:
            ...
            description: "{{ $value }} Cilium agent pods are not running, affecting cilium routing."
        - alert: CiliumHighDropRate
          ...
          annotations:
            ...
            description: "Cilium is dropping {{ $value }} packets/sec. Check cilium routing configuration."
...
Create a Grafana dashboard for cilium routing:
