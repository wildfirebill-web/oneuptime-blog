# Safely Updating the Calico BGPConfiguration Resource in Kubernetes

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Calico, Kubernetes, BGP

Description: A step-by-step guide to modifying Calico BGPConfiguration resources in production without causing downtime or connectivity issues.

---

## Introduction

Updating Calico resources in a running cluster requires care. A misconfigured BGPConfiguration can disrupt networking, drop traffic, or break BGP peerings. This guide covers a safe workflow for modifying BGPConfiguration resources in production.
...
This post provides a repeatable process you can follow every time you need to update a BGPConfiguration resource, whether it is a minor tuning change or a significant configuration shift.
...
- The current BGPConfiguration manifest stored in version control
...
calicoctl get bgpconfiguration -o yaml > bgpconfiguration-backup.yaml

# Store the backup safely
cp bgpconfiguration-backup.yaml bgpconfiguration-backup-$(date +%Y%m%d%H%M%S).yaml
```
...
Open your BGPConfiguration manifest and make the desired changes. Use `diff` to verify exactly what will change:

```bash
# Compare current live state with your updated manifest
diff <(calicoctl get bgpconfiguration -o yaml) bgpconfiguration.yaml
```
...
calicoctl apply -f bgpconfiguration-backup.yaml
...
Updating Calico BGPConfiguration resources safely requires a disciplined approach: backup first, review the diff, apply with validation, and monitor immediately. Always keep your rollback manifest accessible and test changes in a non-production environment before applying them to production clusters.
