# Safely Updating the Calico IPPool Resource in Kubernetes

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Calico, Kubernetes, IPAM

Description: A step-by-step guide to modifying Calico IPPool resources in production without causing downtime or connectivity issues.

---

## Introduction

Updating Calico resources in a running cluster requires care. A misconfigured IPPool can disrupt networking, drop traffic, or break BGP peerings. This guide covers a safe workflow for modifying IPPool resources in production.
...
This post provides a repeatable process you can follow every time you need to update a IPPool resource, whether it is a minor tuning change or a significant configuration shift.
...
- The current IPPool manifest stored in version control
...
cp ippool-backup.yaml ippool-backup-$(date +%Y%m%d%H%M%S).yaml
```
...
Open your IPPool manifest and make the desired changes. Use `diff` to verify exactly what will change:

```bash
# Compare current live state with your updated manifest
diff <(calicoctl get ippool -o yaml) ippool.yaml
```
...
Updating Calico IPPool resources safely requires a disciplined approach: backup first, review the diff, apply with validation, and monitor immediately. Always keep your rollback manifest accessible and test changes in a non-production environment before applying them to production clusters.
