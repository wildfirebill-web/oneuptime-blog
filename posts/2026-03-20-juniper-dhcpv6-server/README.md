# How to Configure DHCPv6 Server on Juniper

Author: [nawazdhandala](https://www.github.com/nawazdhandala)

Tags: Juniper, DHCPv6, IPv6, Junos, DHCP

Description: Configure a DHCPv6 server on Juniper devices to assign IPv6 addresses and options to downstream clients.

## Overview

Configure a DHCPv6 server on Juniper devices to assign IPv6 addresses and options to downstream clients.
...
How to Configure DHCPv6 Server on Juniper follows Juniper's hierarchical configuration syntax. IPv6 configuration under `family inet6` is analogous to IPv4's `family inet`. Always commit changes carefully with `commit check` before `commit`, and use `rollback` if issues arise.
