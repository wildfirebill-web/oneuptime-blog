# Using the Calico IPAMConfiguration Resource in Production Clusters
...
Knowing how to create a Calico IPAMConfiguration resource is only the first step. Using it effectively in production requires understanding common patterns, combining it with other Calico resources, and adapting it to real-world scenarios.

This guide presents practical use cases for the IPAMConfiguration resource, drawn from common production Kubernetes deployments. Each example includes guidance and an explanation of why the configuration is structured that way.

Whether you are running a small cluster or a large multi-tenant environment, these patterns will help you get the most out of the IPAMConfiguration resource.

## Prerequisites

- A running Kubernetes cluster with Calico (v3.26+)
- `kubectl` and `calicoctl` installed
- Basic understanding of the IPAMConfiguration resource fields (see our creation guide)
...
The IPAMConfiguration resource works together with other Calico resources. Here is how to verify the combined effect:
...
After configuring the IPAMConfiguration resource for your production use case, run a comprehensive check:
