# Validating Native Routing in Cilium
...
# Specifically check settings related to native routing in cilium
...
Deploy workloads that specifically test native routing in cilium:

```yaml
# validation-workload.yaml
# Test deployment for native routing in cilium validation
apiVersion: apps/v1
kind: Deployment
metadata:
...
Confirm metrics are being collected for native routing in cilium:
...
```bash
echo "=== Native Routing in Cilium Validation Summary ==="
