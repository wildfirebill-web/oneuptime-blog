# Using the Calico StagedNetworkPolicy Resource in Production Clusters

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Calico, Kubernetes, Network Policy, Staged Policy

Description: Practical patterns and real-world examples for using Calico StagedNetworkPolicy resources effectively in production Kubernetes environments.

---

## Introduction

Knowing how to create a Calico StagedNetworkPolicy resource is only the first step. Using it effectively in production requires understanding common patterns, combining it with other Calico resources, and adapting it to real-world scenarios.

This guide presents practical use cases for the StagedNetworkPolicy resource, drawn from common production Kubernetes deployments. Each example includes guidance and an explanation of why the configuration is structured that way.

Whether you are running a small cluster or a large multi-tenant environment, these patterns will help you get the most out of the StagedNetworkPolicy resource.

## Prerequisites

- A running Kubernetes cluster with Calico (v3.26+)
- `kubectl` and `calicoctl` installed
- Basic understanding of the StagedNetworkPolicy resource fields (see our creation guide)

## Pattern 1: Default Configuration for Small Clusters

For clusters with fewer than 50 nodes, a straightforward StagedNetworkPolicy configuration works well:

```bash
# Verify current StagedNetworkPolicy resources
...
The StagedNetworkPolicy resource works together with other Calico resources. Here is how to verify the combined effect:
...
Set up ongoing monitoring for your StagedNetworkPolicy resources:
...
After configuring the StagedNetworkPolicy resource for your production use case, run a comprehensive check:
