...
Deploy workloads that specifically test disadvantages of native routing in cilium:

```yaml
# validation-workload.yaml
# Test deployment for disadvantages of native routing in cilium validation
apiVersion: apps/v1
kind: Deployment
metadata:
...
Confirm metrics are being collected for disadvantages of native routing in cilium:
...
```bash
echo "=== Disadvantages of Native Routing in Cilium Validation Summary ==="
