...
The `terraform state push` command is a powerful but potentially dangerous tool for uploading state to remote backends. Its primary uses are state recovery, backend migration, and fixing corrupted state. Always back up the remote state before pushing, verify lineage and serial numbers, and run `terraform plan` afterward to confirm consistency. Treat it as an emergency tool rather than a routine operation. For the safe, read-only counterpart, see [terraform state pull](https://oneuptime.com/blog/post/2026-02-23-terraform-state-pull-command/view). For routine state modifications, prefer [terraform state mv](https://oneuptime.com/blog/post/2026-02-23-terraform-state-mv-command/view) and [terraform state rm](https://oneuptime.com/blog/post/2026-02-23-terraform-state-rm-command/view).
