# Monitoring Masquerade Traffic to Remote Nodes in Cilium

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Cilium, Kubernetes, Networking, Monitoring

Description: Set up monitoring and alerting for masquerading behavior for traffic destined to remote nodes in a Cilium cluster to detect issues before they impact application availability.

---

## Introduction

Monitoring masquerade traffic to remote nodes in cilium provides early warning when configuration drift, resource exhaustion, or component failures affect networking. When pods communicate with pods on remote nodes, Cilium can be configured to masquerade or not masquerade this inter-node traffic. The behavior depends on whether the pod CIDR is natively routable between nodes. In overlay (VXLAN/Geneve) mode, masquerading for inter-node traffic is typically not needed because the overlay handles encapsulation.
...
This guide covers metrics collection, dashboard creation, and alert configuration for masquerade traffic to remote nodes in cilium.
...
## Key Metrics for Masquerade Traffic to Remote Nodes in Cilium
...
Create Prometheus alerts for masquerade traffic to remote nodes in cilium:

```yaml
# cilium-feature-alerts.yaml
# Alerting rules for masquerade traffic to remote nodes in cilium
      ...
      rules:
        - alert: CiliumAgentUnhealthy
          ...
          annotations:
            ...
            description: "{{ $value }} Cilium agent pods are not running, affecting masquerade traffic to remote nodes in cilium."
        - alert: CiliumHighDropRate
          ...
          annotations:
            ...
            description: "Cilium is dropping {{ $value }} packets/sec. Check masquerade traffic to remote nodes in cilium configuration."
...
Create a Grafana dashboard for masquerade traffic to remote nodes in cilium:
...
Monitoring masquerade traffic to remote nodes in cilium requires enabling Prometheus metrics on Cilium components, creating dashboards that show operational health and traffic metrics, configuring alerts for component failures and traffic anomalies, and using Hubble for real-time flow analysis. This multi-layer monitoring approach ensures issues are detected early and diagnosed quickly.
