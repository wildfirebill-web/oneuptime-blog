...
- A AWS Route53 account with DNS management access
- A domain hosted in AWS Route53
...
ExternalDNS deployed via Flux CD provides automated, GitOps-managed DNS record lifecycle for Kubernetes services. By combining ExternalDNS's AWS Route53 provider with Flux CD's reconciliation, you get a system where DNS records are automatically created when services are deployed and removed when they are deleted, with all configuration tracked in Git.
