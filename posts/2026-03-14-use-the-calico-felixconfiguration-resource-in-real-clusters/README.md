# Using the Calico FelixConfiguration Resource in Production Clusters

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Calico, Kubernetes, Felix

Description: Practical patterns and real-world examples for using Calico FelixConfiguration resources effectively in production Kubernetes environments.

---

## Introduction

Knowing how to create a Calico FelixConfiguration resource is only the first step. Using it effectively in production requires understanding common patterns, combining it with other Calico resources, and adapting it to real-world scenarios.

This guide presents practical use cases for the FelixConfiguration resource, drawn from common production Kubernetes deployments. Each example includes guidance and an explanation of why the configuration is structured that way.

Whether you are running a small cluster or a large multi-tenant environment, these patterns will help you get the most out of the FelixConfiguration resource.
...
For clusters with fewer than 50 nodes, a straightforward FelixConfiguration configuration works well:

```bash
# Verify current FelixConfiguration resources
...
The FelixConfiguration resource works together with other Calico resources. Here is how to verify the combined effect:
...
Set up ongoing monitoring for your FelixConfiguration resources:

```bash
# Watch for changes to FelixConfiguration resources
kubectl get felixconfiguration.projectcalico.org -w
...
After configuring the FelixConfiguration resource for your production use case, run a comprehensive check:
...
- Verify the resource is correctly applied: `calicoctl get felixconfiguration -o yaml`.
