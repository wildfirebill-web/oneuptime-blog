# Monitoring Cilium Networking Concepts
...
This guide covers metrics collection, dashboard creation, and alert configuration for cilium networking concepts.
...
## Key Metrics for Cilium Networking Concepts

Monitor these Prometheus metrics:

```bash
# Primary metrics to track
# cilium_bpf_map_ops_total - core operational metric
kubectl exec -n kube-system ds/cilium -- cilium metrics list | grep "bpf_map_ops_total"
...
Create Prometheus alerts for cilium networking concepts:

```yaml
# cilium-feature-alerts.yaml
# Alerting rules for cilium networking concepts
      ...
      rules:
        - alert: CiliumAgentUnhealthy
          ...
          annotations:
            ...
            description: "{{ $value }} Cilium agent pods are not running, affecting cilium networking concepts."
        - alert: CiliumHighDropRate
          ...
          annotations:
            ...
            description: "Cilium is dropping {{ $value }} packets/sec. Check cilium networking concepts configuration."
...
Create a Grafana dashboard for cilium networking concepts:
