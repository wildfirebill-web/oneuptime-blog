# Troubleshooting Disadvantages of the Encapsulation Model in Cilium
...
Troubleshooting disadvantages of the encapsulation model in cilium requires understanding how Cilium implements this feature and where failures can occur in the data path. The encapsulation model introduces overhead: VXLAN adds 50 bytes to each packet reducing the effective MTU, encapsulation and decapsulation add CPU overhead on each node, troubleshooting is harder because packet captures show encapsulated packets, and some network appliances cannot inspect encapsulated traffic. For latency-sensitive workloads, native routing mode may be preferable.
...
Examine the Cilium data path for issues related to disadvantages of the encapsulation model in cilium:
