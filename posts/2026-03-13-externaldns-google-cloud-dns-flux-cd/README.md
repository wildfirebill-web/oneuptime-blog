...
- A Google Cloud DNS account with DNS management access
...
ExternalDNS deployed via Flux CD provides automated, GitOps-managed DNS record lifecycle for Kubernetes services. By combining ExternalDNS's Google Cloud DNS provider with Flux CD's reconciliation, you get a system where DNS records are automatically created when services are deployed and removed when they are deleted, with all configuration tracked in Git.
