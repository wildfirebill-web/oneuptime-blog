# Safely Updating the Calico GlobalNetworkSet Resource in Kubernetes

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Calico, Kubernetes

Description: A step-by-step guide to modifying Calico GlobalNetworkSet resources in production without causing downtime or connectivity issues.
...
This post provides a repeatable process you can follow every time you need to update a GlobalNetworkSet resource, whether it is a minor tuning change or a significant configuration shift.
...
- The current GlobalNetworkSet manifest stored in version control
...
calicoctl get globalnetworkset -o yaml > globalnetworkset-backup.yaml

# Store the backup safely
cp globalnetworkset-backup.yaml globalnetworkset-backup-$(date +%Y%m%d%H%M%S).yaml
```
...
Open your GlobalNetworkSet manifest and make the desired changes. Use `diff` to verify exactly what will change:

```bash
# Compare current live state with your updated manifest
diff <(calicoctl get globalnetworkset -o yaml) globalnetworkset.yaml
```
...
calicoctl apply -f globalnetworkset-backup.yaml
...
Updating Calico GlobalNetworkSet resources safely requires a disciplined approach: backup first, review the diff, apply with validation, and monitor immediately. Always keep your rollback manifest accessible and test changes in a non-production environment before applying them to production clusters.
