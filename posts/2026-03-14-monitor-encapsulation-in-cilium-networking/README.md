# Monitoring Encapsulation in Cilium Networking

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Cilium, Kubernetes, Networking, Monitoring

Description: Set up monitoring and alerting for Cilium network encapsulation using VXLAN or Geneve tunnels for inter-node pod communication to detect issues before they impact application availability.

---

## Introduction

Monitoring encapsulation in cilium networking provides early warning when configuration drift, resource exhaustion, or component failures affect networking. Cilium encapsulation wraps pod-to-pod traffic between nodes in VXLAN or Geneve headers. This creates an overlay network where pod IPs are hidden inside the tunnel and the underlying network only sees node-to-node UDP traffic on port 8472 (VXLAN) or 6081 (Geneve). Encapsulation mode is the default and simplest to deploy because it has no requirements on the underlying network.
...
This guide covers metrics collection, dashboard creation, and alert configuration for encapsulation in cilium networking.
...
## Key Metrics for Encapsulation in Cilium Networking
...
Create Prometheus alerts for encapsulation in cilium networking:

```yaml
# cilium-feature-alerts.yaml
# Alerting rules for encapsulation in cilium networking
      ...
      rules:
        - alert: CiliumAgentUnhealthy
          ...
          annotations:
            ...
            description: "{{ $value }} Cilium agent pods are not running, affecting encapsulation in cilium networking."
        - alert: CiliumHighDropRate
          ...
          annotations:
            ...
            description: "Cilium is dropping {{ $value }} packets/sec. Check encapsulation in cilium networking configuration."
...
Create a Grafana dashboard for encapsulation in cilium networking:
