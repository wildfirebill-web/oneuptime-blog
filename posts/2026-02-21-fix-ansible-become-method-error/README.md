# How to Fix Ansible become_method requires become Errors
...
### Fix 2: Set in ansible.cfg

```ini
# ansible.cfg - Set privilege escalation defaults
...
### Fix 3: Per-Task become

```yaml
# Apply become to specific tasks only
- hosts: all
  tasks:
    - name: This runs as the SSH user
      command: whoami
...
### Fix 4: Different become_method Options
...
The fix is simple: whenever you use `become_method`, also set `become: yes`. You can set this at the play level, task level, or globally in ansible.cfg. The `become` flag tells Ansible to escalate privileges, and `become_method` tells it how.
