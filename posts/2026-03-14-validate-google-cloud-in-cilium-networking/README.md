# Validating Cilium on Google Cloud

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Cilium, Kubernetes, Networking

Description: Systematically validate that running Cilium networking on Google Cloud (GKE) including GKE dataplane v2 integration and VPC-native routing is correctly configured and functioning as expected in...

---

## Introduction

Validating cilium on google cloud ensures that your Cilium configuration is not only applied but actually working correctly under real traffic conditions. Google Cloud offers native Cilium integration through GKE Dataplane v2, which replaces kube-proxy with Cilium. When running Cilium on GKE, you get eBPF-based networking that integrates with VPC-native pod addressing, Cloud NAT for egress, and Google Cloud network policies. You can also deploy Cilium manually on GCE-based clusters with native routing using VPC routes.
...
# Specifically check settings related to cilium on google cloud
...
Deploy workloads that specifically test cilium on google cloud:

```yaml
# validation-workload.yaml
# Test deployment for cilium on google cloud validation
apiVersion: apps/v1
kind: Deployment
metadata:
...
Confirm metrics are being collected for cilium on google cloud:
...
```bash
echo "=== Cilium on Google Cloud Validation Summary ==="

# 1. Configuration correct
echo "1. Configuration:"
cilium config view | grep -E "gke|native|routing" 2>/dev/null | head -5
...
Validating cilium on google cloud requires checking the active configuration matches your intent, running automated connectivity tests, deploying custom test workloads that exercise the specific feature, verifying endpoint health, and confirming metrics collection. A passing validation gives confidence that the feature is working correctly before production traffic flows through it.
