# Safely Updating the Calico CalicoNodeStatus Resource in Kubernetes
...
- The current CalicoNodeStatus manifest stored in version control
...
calicoctl get caliconodestatus -o yaml > caliconodestatus-backup.yaml
...
Open your CalicoNodeStatus manifest and make the desired changes. Use `diff` to verify exactly what will change:

```bash
# Compare current live state with your updated manifest
diff <(calicoctl get caliconodestatus -o yaml) caliconodestatus.yaml
```
