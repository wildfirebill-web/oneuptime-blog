# Safely Updating the Calico GlobalNetworkPolicy Resource in Kubernetes

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Calico, Kubernetes, Network Policy

Description: A step-by-step guide to modifying Calico GlobalNetworkPolicy resources in production without causing downtime or connectivity issues.

---

## Introduction

Updating Calico resources in a running cluster requires care. A misconfigured GlobalNetworkPolicy can disrupt networking, drop traffic, or break BGP peerings. This guide covers a safe workflow for modifying GlobalNetworkPolicy resources in production.
...
This post provides a repeatable process you can follow every time you need to update a GlobalNetworkPolicy resource, whether it is a minor tuning change or a significant configuration shift.
...
- The current GlobalNetworkPolicy manifest stored in version control
...
calicoctl get globalnetworkpolicy -o yaml > globalnetworkpolicy-backup.yaml

# Store the backup safely
cp globalnetworkpolicy-backup.yaml globalnetworkpolicy-backup-$(date +%Y%m%d%H%M%S).yaml
```
...
Open your GlobalNetworkPolicy manifest and make the desired changes. Use `diff` to verify exactly what will change:

```bash
# Compare current live state with your updated manifest
diff <(calicoctl get globalnetworkpolicy -o yaml) globalnetworkpolicy.yaml
```
...
calicoctl apply -f globalnetworkpolicy-backup.yaml
...
Updating Calico GlobalNetworkPolicy resources safely requires a disciplined approach: backup first, review the diff, apply with validation, and monitor immediately. Always keep your rollback manifest accessible and test changes in a non-production environment before applying them to production clusters.
