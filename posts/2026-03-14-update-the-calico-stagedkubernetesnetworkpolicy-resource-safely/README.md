# Safely Updating the Calico StagedKubernetesNetworkPolicy Resource in Kubernetes

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Calico, Kubernetes, Network Policy, Staged Policy

Description: A step-by-step guide to modifying Calico StagedKubernetesNetworkPolicy resources in production without causing downtime or connectivity issues.

---

## Introduction

Updating Calico resources in a running cluster requires care. A misconfigured StagedKubernetesNetworkPolicy can disrupt networking, drop traffic, or break BGP peerings. This guide covers a safe workflow for modifying StagedKubernetesNetworkPolicy resources in production.
...
This post provides a repeatable process you can follow every time you need to update a StagedKubernetesNetworkPolicy resource, whether it is a minor tuning change or a significant configuration shift.
...
- The current StagedKubernetesNetworkPolicy manifest stored in version control
...
calicoctl get stagedkubernetesnetworkpolicy -o yaml > stagedkubernetesnetworkpolicy-backup.yaml

# Store the backup safely
cp stagedkubernetesnetworkpolicy-backup.yaml stagedkubernetesnetworkpolicy-backup-$(date +%Y%m%d%H%M%S).yaml
```
...
Open your StagedKubernetesNetworkPolicy manifest and make the desired changes. Use `diff` to verify exactly what will change:

```bash
# Compare current live state with your updated manifest
diff <(calicoctl get stagedkubernetesnetworkpolicy -o yaml) stagedkubernetesnetworkpolicy.yaml
```
...
calicoctl apply -f stagedkubernetesnetworkpolicy-backup.yaml
...
Updating Calico StagedKubernetesNetworkPolicy resources safely requires a disciplined approach: backup first, review the diff, apply with validation, and monitor immediately. Always keep your rollback manifest accessible and test changes in a non-production environment before applying them to production clusters.
