# How to Use Ansible ping Module for Connectivity Testing
...
The `ansible.builtin.ping` module is the simplest way to verify that Ansible can connect to a host and execute tasks. Despite its name, it does not send ICMP pings. Instead, it connects to the host using the configured connection method (usually SSH), runs a small Python script, and returns "pong" if successful.
...
- name: Check response
  ansible.builtin.debug:
    msg: "Host returned: {{ result.ping }}"
  # Output: "Host returned: alive"
```

## Troubleshooting with Ping

```bash
# Verbose ping for debugging connection issues
