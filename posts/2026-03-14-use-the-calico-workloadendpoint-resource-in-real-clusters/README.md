# Using the Calico WorkloadEndpoint Resource in Production Clusters
...
Knowing how to create a Calico WorkloadEndpoint resource is only the first step. Using it effectively in production requires understanding common patterns, combining it with other Calico resources, and adapting it to real-world scenarios.

This guide presents practical use cases for the WorkloadEndpoint resource, drawn from common production Kubernetes deployments. Each example includes guidance and an explanation of why the configuration is structured that way.

Whether you are running a small cluster or a large multi-tenant environment, these patterns will help you get the most out of the WorkloadEndpoint resource.

## Prerequisites

- A running Kubernetes cluster with Calico (v3.26+)
- `kubectl` and `calicoctl` installed
- Basic understanding of the WorkloadEndpoint resource fields (see our creation guide)

## Pattern 1: Default Configuration for Small Clusters

For clusters with fewer than 50 nodes, a straightforward WorkloadEndpoint configuration works well:

```bash
# Verify current WorkloadEndpoint resources
...
Then reference these labels in your WorkloadEndpoint manifest's node selectors to apply environment-specific settings.
...
- Monitor memory usage of calico-node pods when managing many WorkloadEndpoint resources

## Pattern 4: Combining with Other Calico Resources

The WorkloadEndpoint resource works together with other Calico resources. Here is how to verify the combined effect:
...
Set up ongoing monitoring for your WorkloadEndpoint resources:

```bash
# Watch for changes to WorkloadEndpoint resources
kubectl get workloadendpoint.projectcalico.org -w
...
After configuring the WorkloadEndpoint resource for your production use case, run a comprehensive check:
...
- Verify the resource is correctly applied: `calicoctl get workloadendpoint -o yaml`.
