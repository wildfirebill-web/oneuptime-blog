# Troubleshooting Emergency Recovery in Cilium

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Cilium, Kubernetes, Network Security, Troubleshooting, Disaster Recovery

Description: Learn how to troubleshoot recovery procedures in Cilium for Kubernetes. This guide covers practical resolution techniques with real examples and commands.

---

## Introduction

Troubleshooting recovery procedures issues in Cilium requires a systematic approach to identify whether problems stem from policy misconfiguration, agent health, or network connectivity. This guide provides practical diagnostic steps for the most common issues.

When emergency restoration are not working as expected, the impact can range from broken application connectivity to security policy gaps. Understanding Cilium's diagnostic tools and the typical failure modes helps you resolve issues quickly and minimize downtime.
...
When issues are complex and require deeper analysis, collect a full diagnostic bundle from the Cilium agents for offline investigation.

```bash
# Generate a Cilium bugreport for comprehensive diagnostics
cilium sysdump --output-filename cilium-diag-$(date +%Y%m%d)

# Collect specific endpoint diagnostics
cilium endpoint log <ENDPOINT_ID>

# Check BPF program attachment status across all endpoints
cilium bpf endpoint list

# Review the current state of all BPF maps
cilium bpf policy get --all -n

# Verify datapath connectivity between two endpoints
cilium connectivity test --single-node
```

Diagnostic bundles contain agent logs, endpoint states, BPF map contents, and policy snapshots. These are invaluable when working with the Cilium community or support team to resolve complex issues. Always sanitize the bundle before sharing externally to remove any sensitive cluster information.
