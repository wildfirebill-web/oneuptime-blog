# Safely Updating the Calico BGPPeer Resource in Kubernetes

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Calico, Kubernetes, BGP

Description: A step-by-step guide to modifying Calico BGPPeer resources in production without causing downtime or connectivity issues.

---

## Introduction

Updating Calico resources in a running cluster requires care. A misconfigured BGPPeer can disrupt networking, drop traffic, or break BGP peerings. This guide covers a safe workflow for modifying BGPPeer resources in production.
...
This post provides a repeatable process you can follow every time you need to update a BGPPeer resource, whether it is a minor tuning change or a significant configuration shift.
...
- The current BGPPeer manifest stored in version control
...
cp bgppeer-backup.yaml bgppeer-backup-$(date +%Y%m%d%H%M%S).yaml
```
...
Open your BGPPeer manifest and make the desired changes. Use `diff` to verify exactly what will change:

```bash
# Compare current live state with your updated manifest
diff <(calicoctl get bgppeer -o yaml) bgppeer.yaml
```
...
calicoctl apply -f bgppeer-backup.yaml
...
Updating Calico BGPPeer resources safely requires a disciplined approach: backup first, review the diff, apply with validation, and monitor immediately. Always keep your rollback manifest accessible and test changes in a non-production environment before applying them to production clusters.
