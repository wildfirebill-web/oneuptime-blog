# Fixing FailedCreatePodSandBox Errors in Calico

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Calico, Kubernetes, Troubleshooting, Pod Sandbox

Description: Concrete steps to resolve FailedCreatePodSandBox errors in Kubernetes clusters running Calico, with commands you can apply immediately.
...
This guide provides concrete fix procedures for FailedCreatePodSandBox errors. Each step includes the exact commands to run, with explanations of what each command does and why it resolves the issue.
...
- A Kubernetes cluster experiencing FailedCreatePodSandBox errors
...
## Step 2: Restart the calico-node Pod

```bash
kubectl delete pod -n calico-system <calico-node-pod-on-affected-node>
kubectl get pods -n calico-system -o wide -w | grep <affected-node>
```

## Step 3: Remove Conflicting CNI Configurations

```bash
kubectl debug node/<affected-node> -it --image=busybox -- ls /host/etc/cni/net.d/
# Remove non-Calico CNI configurations if present
```
...
FailedCreatePodSandBox errors in Calico are resolvable once you identify the root cause. Always backup your configuration before making changes, apply fixes methodically, and verify each step before proceeding to the next.
