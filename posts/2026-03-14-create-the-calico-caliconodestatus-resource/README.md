# Creating the Calico CalicoNodeStatus Resource in Kubernetes
...
By the end of this post you will have a working CalicoNodeStatus resource applied to your cluster, with a clear understanding of what each field controls and how to verify that the resource is active.
...
The CalicoNodeStatus resource uses the Calico API group `projectcalico.org/v3`. Before writing the manifest, review the key fields:
...
Create a file named `caliconodestatus.yaml` with the following content:
...
```bash
# List CalicoNodeStatus resources
