# How to Configure IPv6 Guest Network on OpenWrt

Author: [nawazdhandala](https://www.github.com/nawazdhandala)

Tags: OpenWrt, IPv6, Guest Network, WiFi, Isolation

Description: Set up an IPv6-enabled guest WiFi network on OpenWrt with proper isolation and a separate IPv6 prefix.

## Overview

Set up an IPv6-enabled guest WiFi network on OpenWrt with proper isolation and a separate IPv6 prefix.
...
How to Configure IPv6 Guest Network on OpenWrt primarily involves UCI configuration files at `/etc/config/network` and `/etc/config/dhcp`. The key components are odhcp6c (WAN DHCPv6 client) and odhcpd (LAN DHCPv6 server + RA). Always check logs with `logread` when troubleshooting IPv6 issues.
