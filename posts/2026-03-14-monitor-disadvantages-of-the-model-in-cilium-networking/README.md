# Monitoring Disadvantages of the Encapsulation Model in Cilium
...
Monitoring disadvantages of the encapsulation model in cilium provides early warning when configuration drift, resource exhaustion, or component failures affect networking. The encapsulation model introduces overhead: VXLAN adds 50 bytes to each packet reducing the effective MTU, encapsulation and decapsulation add CPU overhead on each node, troubleshooting is harder because packet captures show encapsulated packets, and some network appliances cannot inspect encapsulated traffic. For latency-sensitive workloads, native routing mode may be preferable.
...
This guide covers metrics collection, dashboard creation, and alert configuration for disadvantages of the encapsulation model in cilium.
...
## Key Metrics for Disadvantages of the Encapsulation Model in Cilium
...
Create Prometheus alerts for disadvantages of the encapsulation model in cilium:

```yaml
# cilium-feature-alerts.yaml
# Alerting rules for disadvantages of the encapsulation model in cilium
      ...
      rules:
        - alert: CiliumAgentUnhealthy
          ...
          annotations:
            ...
            description: "{{ $value }} Cilium agent pods are not running, affecting disadvantages of the encapsulation model in cilium."
        - alert: CiliumHighDropRate
          ...
          annotations:
            ...
            description: "Cilium is dropping {{ $value }} packets/sec. Check disadvantages of the encapsulation model in cilium configuration."
...
Create a Grafana dashboard for disadvantages of the encapsulation model in cilium:
...
Monitoring disadvantages of the encapsulation model in cilium requires enabling Prometheus metrics on Cilium components, creating dashboards that show operational health and traffic metrics, configuring alerts for component failures and traffic anomalies, and using Hubble for real-time flow analysis. This multi-layer monitoring approach ensures issues are detected early and diagnosed quickly.
