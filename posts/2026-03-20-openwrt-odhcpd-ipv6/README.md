# How to Configure odhcpd for IPv6 on OpenWrt
...
Configure odhcpd, OpenWrt's DHCPv6 server and RA daemon, to provide IPv6 addressing to LAN clients.
...
How to Configure odhcpd for IPv6 on OpenWrt primarily involves UCI configuration files at `/etc/config/network` and `/etc/config/dhcp`. The key components are odhcp6c (WAN DHCPv6 client) and odhcpd (LAN DHCPv6 server + RA). Always check logs with `logread` when troubleshooting IPv6 issues.
