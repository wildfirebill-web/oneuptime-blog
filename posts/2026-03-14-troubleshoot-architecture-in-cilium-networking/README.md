# Troubleshooting Cilium Networking Architecture
...
Troubleshooting cilium networking architecture requires understanding how Cilium implements this feature and where failures can occur in the data path. Cilium architecture consists of several components: the Cilium agent (runs as a DaemonSet on each node), the Cilium operator (handles cluster-wide operations), Hubble (observability layer), and eBPF programs (loaded into the kernel). The agent manages endpoints, loads BPF programs, and syncs with the Kubernetes API. The operator handles IPAM, node management, and garbage collection.
...
Examine the Cilium data path for issues related to cilium networking architecture:
