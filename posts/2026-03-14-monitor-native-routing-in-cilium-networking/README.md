# Monitoring Native Routing in Cilium
...
Monitoring native routing in cilium provides early warning when configuration drift, resource exhaustion, or component failures affect networking. Native routing mode in Cilium disables tunnel encapsulation and instead programs routes directly into the Linux kernel routing table. The underlying network must be capable of routing pod CIDRs between nodes, either through static routes, BGP, or cloud provider routing tables. Native routing provides the best performance because it avoids encapsulation overhead.
...
This guide covers metrics collection, dashboard creation, and alert configuration for native routing in cilium.
...
## Key Metrics for Native Routing in Cilium
...
Create Prometheus alerts for native routing in cilium:

```yaml
# cilium-feature-alerts.yaml
# Alerting rules for native routing in cilium
      ...
      rules:
        - alert: CiliumAgentUnhealthy
          ...
          annotations:
            ...
            description: "{{ $value }} Cilium agent pods are not running, affecting native routing in cilium."
        - alert: CiliumHighDropRate
          ...
          annotations:
            ...
            description: "Cilium is dropping {{ $value }} packets/sec. Check native routing in cilium configuration."
...
Create a Grafana dashboard for native routing in cilium:
