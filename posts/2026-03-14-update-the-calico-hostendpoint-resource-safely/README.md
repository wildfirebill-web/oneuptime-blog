# Safely Updating the Calico HostEndpoint Resource in Kubernetes
...
Updating Calico resources in a running cluster requires care. A misconfigured HostEndpoint can disrupt networking, drop traffic, or break BGP peerings. This guide covers a safe workflow for modifying HostEndpoint resources in production.
...
This post provides a repeatable process you can follow every time you need to update a HostEndpoint resource, whether it is a minor tuning change or a significant configuration shift.
...
- The current HostEndpoint manifest stored in version control
...
calicoctl get hostendpoint -o yaml > hostendpoint-backup.yaml

# Store the backup safely
cp hostendpoint-backup.yaml hostendpoint-backup-$(date +%Y%m%d%H%M%S).yaml
```
...
Open your HostEndpoint manifest and make the desired changes. Use `diff` to verify exactly what will change:

```bash
# Compare current live state with your updated manifest
diff <(calicoctl get hostendpoint -o yaml) hostendpoint.yaml
```
...
calicoctl apply -f hostendpoint-backup.yaml
...
Updating Calico HostEndpoint resources safely requires a disciplined approach: backup first, review the diff, apply with validation, and monitor immediately. Always keep your rollback manifest accessible and test changes in a non-production environment before applying them to production clusters.
