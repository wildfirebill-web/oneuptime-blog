# Using the Calico IPPool Resource in Production Clusters

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Calico, Kubernetes, IPAM

Description: Practical patterns and real-world examples for using Calico IPPool resources effectively in production Kubernetes environments.

---

## Introduction

Knowing how to create a Calico IPPool resource is only the first step. Using it effectively in production requires understanding common patterns, combining it with other Calico resources, and adapting it to real-world scenarios.

This guide presents practical use cases for the IPPool resource, drawn from common production Kubernetes deployments. Each example includes guidance and an explanation of why the configuration is structured that way.

Whether you are running a small cluster or a large multi-tenant environment, these patterns will help you get the most out of the IPPool resource.

## Prerequisites

- A running Kubernetes cluster with Calico (v3.26+)
- `kubectl` and `calicoctl` installed
- Basic understanding of the IPPool resource fields (see our creation guide)

## Pattern 1: Default Configuration for Small Clusters

For clusters with fewer than 50 nodes, a straightforward IPPool configuration works well:
...
Then reference these labels in your IPPool manifest's node selectors to apply environment-specific settings.
...
- Monitor memory usage of calico-node pods when managing many IPPool resources

## Pattern 4: Combining with Other Calico Resources

The IPPool resource works together with other Calico resources. Here is how to verify the combined effect:
...
Set up ongoing monitoring for your IPPool resources:
...
After configuring the IPPool resource for your production use case, run a comprehensive check:
...
- Verify the resource is correctly applied: `calicoctl get ippool -o yaml`.
