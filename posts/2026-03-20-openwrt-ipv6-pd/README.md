# How to Configure IPv6 Prefix Delegation on OpenWrt

Author: [nawazdhandala](https://www.github.com/nawazdhandala)

Tags: OpenWrt, IPv6, Prefix Delegation, DHCPv6-PD, UCI

Description: Set up DHCPv6 prefix delegation on OpenWrt to receive and distribute delegated IPv6 prefixes from your ISP.

## Overview

Set up DHCPv6 prefix delegation on OpenWrt to receive and distribute delegated IPv6 prefixes from your ISP.
...
How to Configure IPv6 Prefix Delegation on OpenWrt primarily involves UCI configuration files at `/etc/config/network` and `/etc/config/dhcp`. The key components are odhcp6c (WAN DHCPv6 client) and odhcpd (LAN DHCPv6 server + RA). Always check logs with `logread` when troubleshooting IPv6 issues.
