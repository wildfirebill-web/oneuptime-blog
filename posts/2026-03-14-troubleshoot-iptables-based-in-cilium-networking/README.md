# Troubleshooting iptables-Based Masquerading in Cilium

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Cilium, Kubernetes, Networking

Description: Diagnose and resolve common issues with iptables-based masquerading mode in Cilium networking as an alternative to eBPF-based masquerading using systematic debugging techniques and Cilium...

---

## Introduction

Troubleshooting iptables-based masquerading in cilium requires understanding how Cilium implements this feature and where failures can occur in the data path. Cilium can use iptables for masquerading instead of eBPF. The iptables mode is the legacy approach that provides broader kernel compatibility but lower performance compared to eBPF masquerading. It works by installing iptables MASQUERADE rules in the POSTROUTING chain for traffic leaving the pod CIDR.
...
Examine the Cilium data path for issues related to iptables-based masquerading in cilium:
