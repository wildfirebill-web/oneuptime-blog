# How to Configure IPv6 WAN Interface on OpenWrt

Author: [nawazdhandala](https://www.github.com/nawazdhandala)

Tags: OpenWrt, IPv6, WAN, DHCPv6, UCI

Description: Configure the WAN6 interface on OpenWrt for DHCPv6, SLAAC, or static IPv6 connectivity from your ISP.

## Overview

Configure the WAN6 interface on OpenWrt for DHCPv6, SLAAC, or static IPv6 connectivity from your ISP.
...
How to Configure IPv6 WAN Interface on OpenWrt primarily involves UCI configuration files at `/etc/config/network` and `/etc/config/dhcp`. The key components are odhcp6c (WAN DHCPv6 client) and odhcpd (LAN DHCPv6 server + RA). Always check logs with `logread` when troubleshooting IPv6 issues.
