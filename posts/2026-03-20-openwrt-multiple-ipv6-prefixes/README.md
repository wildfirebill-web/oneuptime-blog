# How to Configure IPv6 with Multiple Upstream Prefixes on OpenWrt

Author: [nawazdhandala](https://www.github.com/nawazdhandala)

Tags: OpenWrt, IPv6, Multiple Prefixes, Multihoming, UCI

Description: Handle multiple delegated IPv6 prefixes from different upstreams on OpenWrt for multihomed or failover configurations.

## Overview

Handle multiple delegated IPv6 prefixes from different upstreams on OpenWrt for multihomed or failover configurations.
...
How to Configure IPv6 with Multiple Upstream Prefixes on OpenWrt primarily involves UCI configuration files at `/etc/config/network` and `/etc/config/dhcp`. The key components are odhcp6c (WAN DHCPv6 client) and odhcpd (LAN DHCPv6 server + RA). Always check logs with `logread` when troubleshooting IPv6 issues.
