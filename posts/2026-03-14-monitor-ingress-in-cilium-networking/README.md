# Monitoring Ingress in Cilium Networking
...
Monitoring ingress in cilium networking provides early warning when configuration drift, resource exhaustion, or component failures affect networking. Cilium provides a built-in Ingress controller and Gateway API implementation powered by Envoy. It handles external HTTP/HTTPS traffic routing to cluster services without requiring a separate ingress controller. Cilium Ingress supports TLS termination, path-based routing, header manipulation, and integrates with Cilium network policies for security.
...
This guide covers metrics collection, dashboard creation, and alert configuration for ingress in cilium networking.
...
## Key Metrics for Ingress in Cilium Networking

Monitor these Prometheus metrics:

```bash
# Primary metrics to track
# cilium_proxy_upstream_reply_seconds - core operational metric
kubectl exec -n kube-system ds/cilium -- cilium metrics list | grep "proxy_upstream_reply_seconds"

# PromQL queries for Grafana panels:

# Panel 1: Operational rate
rate(cilium_proxy_upstream_reply_seconds[5m])
...
Create Prometheus alerts for ingress in cilium networking:

```yaml
# cilium-feature-alerts.yaml
# Alerting rules for ingress in cilium networking
      ...
      rules:
        - alert: CiliumAgentUnhealthy
          ...
          annotations:
            ...
            description: "{{ $value }} Cilium agent pods are not running, affecting ingress in cilium networking."
        - alert: CiliumHighDropRate
          ...
          annotations:
            ...
            description: "Cilium is dropping {{ $value }} packets/sec. Check ingress in cilium networking configuration."
...
Create a Grafana dashboard for ingress in cilium networking:
