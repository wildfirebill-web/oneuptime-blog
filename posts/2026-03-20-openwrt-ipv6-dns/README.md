# How to Configure IPv6 DNS on OpenWrt
...
Configure dnsmasq on OpenWrt to resolve AAAA records, serve DNS to IPv6 clients, and use IPv6 DNS upstream servers.
...
How to Configure IPv6 DNS on OpenWrt primarily involves UCI configuration files at `/etc/config/network` and `/etc/config/dhcp`. The key components are odhcp6c (WAN DHCPv6 client) and odhcpd (LAN DHCPv6 server + RA). Always check logs with `logread` when troubleshooting IPv6 issues.
