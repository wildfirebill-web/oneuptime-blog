# How to Roll Out Staged Kubernetes NetworkPolicy in Calico Safely

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Calico, Kubernetes, Network Policy, Staged Policies

Description: A phased rollout strategy for Staged Kubernetes NetworkPolicy in Calico that prevents outages.

---

## Introduction

Staged Kubernetes NetworkPolicy is an advanced Calico feature that provides fine-grained network security controls using the `projectcalico.org/v3` API. This guide covers how to roll out Staged K8s NetworkPolicy effectively in your Kubernetes cluster.
...
This guide provides production-tested patterns for roll out Staged K8s NetworkPolicy, including YAML examples, CLI commands, and troubleshooting techniques.
...
kind: NetworkPolicy
metadata:
  name: roll-out-staged-k8s-networkpolicy
...
calicoctl apply -f roll-out-staged-k8s-networkpolicy.yaml
...
Roll Out Staged K8s NetworkPolicy in Calico requires careful attention to policy ordering, selector syntax, and bidirectional traffic rules. Use the patterns in this guide as a starting point, adapt them to your specific requirements, and always validate changes in a staging environment before applying to production. Consistent logging and monitoring will help you detect and resolve issues quickly when they occur.
