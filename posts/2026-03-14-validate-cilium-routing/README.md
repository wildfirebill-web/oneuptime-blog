# Validating Cilium Routing

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Cilium, Kubernetes, Networking

Description: Systematically validate that Cilium routing configuration and operations including tunnel and native routing modes is correctly configured and functioning as expected in your Kubernetes cluster.

---

## Introduction

Validating cilium routing ensures that your Cilium configuration is not only applied but actually working correctly under real traffic conditions. Cilium routing determines how pod-to-pod traffic is forwarded between nodes. In tunnel mode, Cilium encapsulates packets in VXLAN or Geneve headers. In native routing mode, Cilium programs routes directly into the Linux kernel routing table and relies on the underlying network to forward packets between nodes.
...
# Specifically check settings related to cilium routing
cilium config view | grep -E "routing|tunnel|native"
...
Deploy workloads that specifically test cilium routing:

```yaml
# validation-workload.yaml
# Test deployment for cilium routing validation
apiVersion: apps/v1
kind: Deployment
metadata:
...
Confirm metrics are being collected for cilium routing:
...
```bash
echo "=== Cilium Routing Validation Summary ==="

# 1. Configuration correct
echo "1. Configuration:"
cilium config view | grep -E "routing|tunnel|native" 2>/dev/null | head -5
...
Validating cilium routing requires checking the active configuration matches your intent, running automated connectivity tests, deploying custom test workloads that exercise the specific feature, verifying endpoint health, and confirming metrics collection. A passing validation gives confidence that the feature is working correctly before production traffic flows through it.
