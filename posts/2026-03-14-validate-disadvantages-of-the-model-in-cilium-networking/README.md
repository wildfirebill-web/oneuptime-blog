# Validating Disadvantages of the Encapsulation Model in Cilium
...
Validating disadvantages of the encapsulation model in cilium ensures that your Cilium configuration is not only applied but actually working correctly under real traffic conditions. The encapsulation model introduces overhead: VXLAN adds 50 bytes to each packet reducing the effective MTU, encapsulation and decapsulation add CPU overhead on each node, troubleshooting is harder because packet captures show encapsulated packets, and some network appliances cannot inspect encapsulated traffic. For latency-sensitive workloads, native routing mode may be preferable.
...
# Specifically check settings related to disadvantages of the encapsulation model in cilium
...
Deploy workloads that specifically test disadvantages of the encapsulation model in cilium:

```yaml
# validation-workload.yaml
# Test deployment for disadvantages of the encapsulation model in cilium validation
apiVersion: apps/v1
kind: Deployment
metadata:
...
Confirm metrics are being collected for disadvantages of the encapsulation model in cilium:
...
```bash
echo "=== Disadvantages of the Encapsulation Model in Cilium Validation Summary ==="

# 1. Configuration correct
echo "1. Configuration:"
cilium config view | grep -E "mtu|tunnel|bpf" 2>/dev/null | head -5
...
Validating disadvantages of the encapsulation model in cilium requires checking the active configuration matches your intent, running automated connectivity tests, deploying custom test workloads that exercise the specific feature, verifying endpoint health, and confirming metrics collection. A passing validation gives confidence that the feature is working correctly before production traffic flows through it.
