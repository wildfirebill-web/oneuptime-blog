# Safely Updating the Calico BGPFilter Resource in Kubernetes

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Calico, Kubernetes, BGP

Description: A step-by-step guide to modifying Calico BGPFilter resources in production without causing downtime or connectivity issues.

---

## Introduction

Updating Calico resources in a running cluster requires care. A misconfigured BGPFilter can disrupt networking, drop traffic, or break BGP peerings. This guide covers a safe workflow for modifying BGPFilter resources in production.
...
This post provides a repeatable process you can follow every time you need to update a BGPFilter resource, whether it is a minor tuning change or a significant configuration shift.
...
- The current BGPFilter manifest stored in version control
...
calicoctl get bgpfilter -o yaml > bgpfilter-backup.yaml

# Store the backup safely
cp bgpfilter-backup.yaml bgpfilter-backup-$(date +%Y%m%d%H%M%S).yaml
```
...
Open your BGPFilter manifest and make the desired changes. Use `diff` to verify exactly what will change:

```bash
# Compare current live state with your updated manifest
diff <(calicoctl get bgpfilter -o yaml) bgpfilter.yaml
```
...
calicoctl apply -f bgpfilter-backup.yaml
...
Updating Calico BGPFilter resources safely requires a disciplined approach: backup first, review the diff, apply with validation, and monitor immediately. Always keep your rollback manifest accessible and test changes in a non-production environment before applying them to production clusters.
