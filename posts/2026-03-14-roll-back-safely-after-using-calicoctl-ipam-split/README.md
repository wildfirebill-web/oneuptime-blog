...
Recovery from `calicoctl ipam split` operations depends on whether the command modified state. For read-only commands, focus on correct interpretation. For state-modifying commands, verify IPAM health and remediate any issues using the check and show commands.
