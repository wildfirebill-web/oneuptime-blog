# Safely Updating the Calico IPReservation Resource in Kubernetes

Author: [nawazdhandala](https://github.com/nawazdhandala)

Tags: Calico, Kubernetes, IPAM

Description: A step-by-step guide to modifying Calico IPReservation resources in production without causing downtime or connectivity issues.
...
- The current IPReservation manifest stored in version control
...
calicoctl get ipreservation -o yaml > ipreservation-backup.yaml

# Store the backup safely
cp ipreservation-backup.yaml ipreservation-backup-$(date +%Y%m%d%H%M%S).yaml
```
...
Open your IPReservation manifest and make the desired changes. Use `diff` to verify exactly what will change:

```bash
# Compare current live state with your updated manifest
diff <(calicoctl get ipreservation -o yaml) ipreservation.yaml
```
...
calicoctl apply -f ipreservation-backup.yaml
...
Updating Calico IPReservation resources safely requires a disciplined approach: backup first, review the diff, apply with validation, and monitor immediately. Always keep your rollback manifest accessible and test changes in a non-production environment before applying them to production clusters.
